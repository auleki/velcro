...
<head>
    ...
    <title>{{$sent_report->report_title}}</title>
    <style type="text/css">
    @font-face{
        font-family:europa;
...
<body>
    <div style="margin-top:20px">
        ...
        <div class="content">
            <h4 style="width: 100%;margin:0px">{{$contact->fname}} from {{$contact->company}}</h4>
            <span style="width: 100%; font-size: 10px">{{ $date }}</span>
            
          </div>
        <div class="h2">
            {{ $sent_report ? $sent_report->report_title : '' }}
        </div>
        ...
        <div class="content margin-left">
            ...
            <div style="margin-bottom:20px">
                ...
                <textarea name="" id="" cols="30" rows="5" style="font-size: 15px;font-weight: lighter;display:block;margin:0px 30px;">{{ $texts[$i]->response }}</textarea>
            </div>
            <hr style="width:90%">
            @endfor
        </div>
        ...
        <div class="content margin-left">
            ...
            <div style="margin-bottom:20px">
                <h5 style="display:block;padding:0px 30px;padding-top:30px">{{ $metrics[$j]->title }} <span class="{{$metrics[$j]->reqd == 'true' ? 'reqd':'null'}}">&lowast;</span></h5>
                <p style="font-size: 15px;font-weight: lighter;display:block;padding:0px 30px">
                    {{ $metrics[$j]->desc }}
                </p>
                <div>
                    <table style="width: 100%;padding: 0px 30px;">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $metrics[$j]->kpis[$k]->name }}</td>
                                <td>{{ $metrics[$j]->kpis[$k]->format }}</td>
                                <td><textarea name="" id="" cols="30" rows="10">{{$metrics[$j]->kpis[$k]->value}}</textarea></td>
                            </tr>
                            @endfor
                        </tbody>
                    </table>
